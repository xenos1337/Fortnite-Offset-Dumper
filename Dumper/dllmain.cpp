...
BOOL APIENTRY DllMain(HMODULE module, DWORD reason, LPVOID reserved) {
	if (reason == DLL_PROCESS_ATTACH) {
		try 
		{
			...
			std::cout << "Dumper by xenos#1337 & android#1337" << std::endl;
			std::cout << "-------------------" << std::endl;
			std::cout << "uWorld: " << "0x" << std::hex << (uintptr_t)reinterpret_cast<decltype(uWorld)>((PBYTE)((UINT_PTR)(FindPattern("\x48\x89\x05\x00\x00\x00\x00\x48\x8B\x4B\x78", "xxx????xxxx")) + *(PINT)((UINT_PTR)(FindPattern("\x48\x89\x05\x00\x00\x00\x00\x48\x8B\x4B\x78", "xxx????xxxx"))+((7)-sizeof(INT))) + (7))) - (uintptr_t)GetModuleHandle(L"FortniteClient-Win64-Shipping.exe") << std::endl;
			...
			std::cout << "ProjectWorldToScreen: " << "0x" << std::hex << (uintptr_t)reinterpret_cast<decltype(ProjectWorldToScreen)>((PBYTE)((UINT_PTR)(FindPattern("\xE8\x00\x00\x00\x00\x41\x88\x07\x48\x83\xC4\x30\xE8\x00\x00\x00\x00\x48\x8B\x5C\x24\x00\x41\x88\x07\x48\x83\xC4\x30", "x????xxxx?xxxxxxx")) + *(PINT)((UINT_PTR)(FindPattern("\xE8\x00\x00\x00\x00\x48\x8B\x5C\x24\x00\x41\x88\x07\x48\x83\xC4\x30", "x????xxxx?xxxxxxx")) + ((7) - sizeof(INT))) + (7))) - (uintptr_t)GetModuleHandle(L"FortniteClient-Win64-Shipping.exe") << std::endl;
